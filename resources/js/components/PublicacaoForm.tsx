...
export function PublicacaoForm({
  ...
  const nomeAbrevOptions = useMemo(
    () => locaisFiltrados.filter((o) => o.nome_abreviado).map((o) => ({ id: o.id, nome: o.nome_abreviado! })),
    [locaisFiltrados],
  );
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <FormSection title="Conteúdo">
          <Field label="Resumo" required error={errors.resumo}>
            ...
            >
              {resumoLen < RESUMO_MIN
                ? `${resumoLen}/${RESUMO_MIN} mín`
                : `${resumoLen.toLocaleString('pt-BR')}/${RESUMO_MAX.toLocaleString('pt-BR')} máx`}
            </p>
          </Field>

          <Field label="Palavras-chave" error={errors.palavras_chave as unknown as string}>
            <PalavraChaveInput value={data.palavras_chave} onChange={(v) => setData('palavras_chave', v)} />
          </Field>
              ...
              <Popover open={estadoOpen} onOpenChange={setEstadoOpen}>
                <PopoverTrigger asChild>
                  <Button
                    ...
                    variant="outline"
                    role="combobox"
                    className={cn('w-full justify-between font-normal', !filterEstado && 'text-muted-foreground')}
                  >
      ...
      <Dialog open={createLocalOpen} onOpenChange={setCreateLocalOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Preencha os campos abaixo. Os periódicos existentes que correspondem ao que você digitar aparecerão automaticamente — verifique antes de
              criar.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex flex-col gap-3">
            <div className="flex flex-col gap-1.5">
              ...
              <Input
                value={createLocalForm.nome}
                onChange={(e) => setCreateLocalForm((f) => ({ ...f, nome: e.target.value }))}
                placeholder="Nome completo do periódico"
                autoFocus
              />
            </div>
            <div className="flex flex-col gap-1.5">
              ...
              <Input
                value={createLocalForm.nome_abreviado}
                onChange={(e) => setCreateLocalForm((f) => ({ ...f, nome_abreviado: e.target.value }))}
                placeholder="Ex: Rev. Bras. Ed."
              />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1.5">
                <Label>ISSN</Label>
                <Input
                  value={createLocalForm.issn}
                  onChange={(e) => setCreateLocalForm((f) => ({ ...f, issn: e.target.value }))}
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <Select
                  value={createLocalForm.estado || '__none__'}
                  onValueChange={(v) => setCreateLocalForm((f) => ({ ...f, estado: v === '__none__' ? '' : v }))}
                >
                  <SelectTrigger>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setCreateLocalOpen(false)}>
              Cancelar
            </Button>
            <Button
              disabled={!createLocalForm.nome.trim()}
              onClick={() => {
                if (createLocalForm.nome.trim()) setCreateLocalConfirmOpen(true);
              }}
            >
              Criar periódico
            </Button>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <br />
              Verifique se ele já não existe com outra grafia para evitar duplicatas.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setCreateLocalConfirmOpen(false)} disabled={createLocalSaving}>
              Cancelar
            </Button>
